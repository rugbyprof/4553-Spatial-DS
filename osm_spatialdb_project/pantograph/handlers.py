...
class PantographHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, raw_message):
        ...
        elif event_type == "lat_lon":
            self.on_latlon(message)
        else:
            event_callbacks = {
    ...
    def map_init(self,lon,lat,zoom):
        self.do_operation("initMap",lat=lat,lon=lon,zoom=zoom)

    def map_pan(self,direction,distance):
        self.do_operation("panMap",direction=direction,distance=distance)

    def map_done_loading(self):
        self.do_operation("hideMapLoader")

    def map_fitBounds(self,minLon,minLat,maxLon,maxLat):
        self.do_operation("fitBounds",minLon=minLon,minLat=minLat,maxLon=maxLon,maxLat=maxLat)

    def map_click(self):
        print "map clicked"
        self.do_operation("clickMap")

    def on_latlon(self,event):
        print event
